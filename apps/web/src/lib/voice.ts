...
export function speakAsTarik(text: string) {
  return text
    ...
    .replace(/\bI am (an? )?(ai|bot|chatgpt|language model)\b/gi, "tarik hoon")
    .replace(/\bas (an? )?(ai|personal ai|language model)\b/gi, "")
    .replace(/\bpersonal ai\b/gi, "")
    .replace(/\bauto[- ]?reply\b/gi, "")
    .replace(/\bchat ?bot\b/gi, "")
