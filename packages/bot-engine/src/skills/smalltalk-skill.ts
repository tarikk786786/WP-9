...
export class SmalltalkSkill implements SpecialistSkill {
  ...
  public canHandle(ctx: SkillContext): boolean {
    ...
    return (
      /^(hi|hello|hey|suno|kya haal|kaise ho|kaisi ho|wassup|yo|namaste|namaskar|pranam|good morning|good night|gn|gm|bye|alvida|shukriya|thanks|thank you|salam|assalam|asalam|aoa|jazakallah|jazak allah)\b/i.test(
        text,
      ) && text.split(" ").length <= 5
    );
  }
  ...
  public async execute(ctx: SkillContext): Promise<SkillResponse | null> {
    ...
    // Salam handling: reciprocate politely
    if (/^(assalam|asalam|salam|slaam|slm|aoa)\b/i.test(text)) {
      return {
        skillId: this.id,
        replyText: "Walaikum Assalam! Ji khairiyat? Boliye, main kis tarah madad kar sakta hoon?",
        confidence: 95,
      };
    }

    // Namaste / Namaskar handling: reciprocate politely
    if (/^(namaste|namaskar|pranam)\b/i.test(text)) {
      return {
        skillId: this.id,
        replyText: "Namaste! Ji boliye, sab theek thaak? Kis cheez mein madad chahiye?",
        confidence: 95,
      };
    }

    // Universal general greetings
    if (/^(hi|hello|hey|yo)\b/i.test(text)) {
      return {
        skillId: this.id,
        replyText: "Hello! Ji boliye, sab theek thaak? Kis cheez mein madad chahiye?",
        confidence: 90,
      };
    }

    // Universal check-in / wellness
    if (/^(kaise ho|kya haal|kaisi ho)/i.test(text)) {
      return {
        skillId: this.id,
        replyText: "Main theek hoon, bahut shukriya! Aap bataiye sab kaisa chal raha hai?",
        confidence: 95,
      };
    }
    ...
    return {
      skillId: this.id,
      replyText: "Hello! Ji boliye, sab theek thaak? Bataiye main kaise help kar sakta hoon?",
      confidence: 80,
    };
  }
}
